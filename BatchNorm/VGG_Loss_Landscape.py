...
def plot_loss_landscape(iteration,VGG_A_max_curve,
    ...
    plt.savefig('T2.3 VGG_Cmp_BN.png')
...
for lr in lrs:
    ...
    VGG_A_losses.append(train(model, optimizer, criterion, train_loader, val_loader, epochs_n=epo))
    
    print("lr:{0}\tVGG_A_BatchNorm".format(lr))
    model = VGG_A_BatchNorm()
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    criterion = nn.CrossEntropyLoss()
    VGG_A_BN_losses.append(train(model, optimizer, criterion, train_loader, val_loader, epochs_n=epo))
