...
class SLBI_ADAM(Optimizer):
	...
	# Adam 需要参数 beta 和 epsilon
	def __init__ (self, params, lr=required, kappa=1, mu=100, weight_decay=0, momentum=0.9, dampening=0, beta_1=0.9, beta_2=0.99, epsilon=1e-8):
		defaults = dict(lr=lr, kappa=kappa, mu=mu, weight_decay=weight_decay, momentum=momentum, dampening=dampening, beta_1=beta_1, beta_2=beta_2, epsilon=epsilon)
		print('*******************************************')
		for key in defaults:
	...
	def step(self, closure=None):
		...
		for group in self.param_groups:
			...
			for p in group['params']:
				...
				bias_corr1 = 1 - beta_1 ** step
				bias_corr2 = 1 - beta_2 ** step

				exp_avg.mul_(beta_1).add_(d_p, alpha=1-beta_1)
				exp_avg_sq.mul_(beta_2).addcmul_(d_p, d_p, value=1-beta_2)
				denom = (exp_avg_sq.sqrt() / math.sqrt(bias_corr2)).add_(epsilon)
				
				step_size = lr_kappa / bias_corr1
